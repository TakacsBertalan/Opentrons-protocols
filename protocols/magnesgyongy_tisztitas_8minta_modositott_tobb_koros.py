...
metadata = {
    'protocolName': 'DNA Purification - 8 samples - optimalization - B version - 5 rounds',
...
def run(protocol_context):
    ...
    for target in samples:
        ...
        p300_pipette.transfer(90, beads, adjusted_location, new_tip='never')
        p300_pipette.mix(10, 130, adjusted_location)
    ...
    output = [col for col in output_plate.rows()[0][1:2]]
    ...
    for target in samples:
        #p300_pipette.pick_up_tip()
        adjusted_location = modify_tip_position(target, 1.5, 2, "bottom", "bal")
        p300_pipette.transfer(78.3, beads, adjusted_location, new_tip='never')
        p300_pipette.mix(10, 120, adjusted_location)
        p300_pipette.blow_out()
        p300_pipette.drop_tip()
    ...
    #A plate-et nem dobja ki automatikusan
    # On OT-2, you can can only move labware manually, since it doesn’t have a gripper instrument.
    protocol_context.pause("Cseréld ki a plate-eket, harmadik tisztítási lépés jön")
    samples = [col for col in mag_plate.rows()[0][1:2]]
    ...
    output = [col for col in output_plate.rows()[0][2:3]]
    ...
    for target in samples:
        ...
        p300_pipette.mix(10, 80, adjusted_location)
        p300_pipette.blow_out()
        p300_pipette.drop_tip()
    ...
    samples = [col for col in mag_plate.rows()[0][2:3]]
    ...
    for target in samples:
        #p300_pipette.pick_up_tip()
        adjusted_location = modify_tip_position(target, 1.5, 2, "bottom", "bal")
        p300_pipette.transfer(50, beads, adjusted_location, new_tip='never')

        p300_pipette.mix(10, 90, adjusted_location)
        p300_pipette.blow_out()
        p300_pipette.drop_tip()
